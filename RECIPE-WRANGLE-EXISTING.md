...
3. **`data/` is sacred.** Never move files *into* `data/raw` or `data/external` unless they
   ...
   is input (`data/`) or derived (`cache/`/`results/`), **ask** — do not guess.
...
3. **Script-path edits:** the list of read/write paths inside scripts that the moves will
   ...
   edits yet.)
4. **`.gitignore` changes** to add (`cache/`, `results/figures/`, env dirs).
5. **NEEDS HUMAN INPUT** section: every file you couldn't classify, with your best guess and
   the question.
6. **Risks**: large files that will change git status, anything ambiguous, anything
   irreversible.
...
6. Add `data/README.md` (provenance template) and a `README.md` rebuild note if absent.
...
- **Nothing in `data/` was modified** (only additions like `data/README.md`).
...
- Files sit in the correct provenance tiers; the pipeline is numbered; `data/` is untouched
  and protected by `.gitignore`; derived outputs are ignored/untracked.
